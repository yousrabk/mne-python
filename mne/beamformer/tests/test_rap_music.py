...
def test_rap_music_simulated():
    ...
    stc, dipoles = rap_music(sim_evoked, forward_fixed, noise_cov,
                             n_dipoles=n_dipoles)
    _check_dipoles(dipoles, forward_fixed, stc, evoked)

    stc, dipoles, residual = rap_music(sim_evoked, forward_fixed, noise_cov,
                                       n_dipoles=n_dipoles,
                                       return_residual=True)
    _check_dipoles(dipoles, forward_fixed, stc, evoked, residual)

    # Check dipoles for free ori
    stc, dipoles, residual = rap_music(sim_evoked, forward, noise_cov,
                                       n_dipoles=n_dipoles,
                                       return_residual=True)
    _check_dipoles(dipoles, forward_fixed, stc, evoked, residual)

    # Check dipoles for free surface ori
    stc, dipoles, residual = rap_music(sim_evoked, forward_surf_ori,
                                       noise_cov, n_dipoles=n_dipoles,
                                       return_residual=True)
    _check_dipoles(dipoles, forward_fixed, stc, evoked, residual)
